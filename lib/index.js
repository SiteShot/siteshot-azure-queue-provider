 ...
 * @constructor
 */
exports.PublishToJobQueue = function (jobContract, callback) {
...
/**
 * Push an update through
 * @param action
 * @param jobContract
 * @param callback
 * @constructor
 */
exports.PublishToUpdateQueue = function (action, jobContract, callback) {
    jobContract.action = action;
    queueSvc.createMessage(config.Azure.Queues.Updates, JSON.stringify(jobContract), callback);
};
...
/**
 * Checks the queue for scan jobs
 * @param processJobCallback
 * @param noJobCallback
 * @constructor
 */
exports.CheckForScanJobs = function (processJobCallback, noJobCallback) {
    function ProcessJob(message, callback) {
        function _deleteMessage(delMessage) {
            queueSvc.deleteMessage(config.Azure.Queues.Scans, delMessage.messageId, delMessage.popReceipt, function (error, response) {
                if (!error) {
                    logger.debug(delMessage.messageId, "deleted");
                }
            });
    ...
    queueSvc.getMessages(config.Azure.Queues.Scans, {numOfMessages: 1}, function (error, result, response) {
            logger.debug("Have", result.length, "jobs");
 ...
 * @constructor
 */
exports.CheckForScanUpdateJobs = function (processJobCallback, noJobCallback) {
    function ProcessJob(message, callback) {
        function _deleteMessage(delMessage) {
